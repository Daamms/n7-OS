...
void initialise_paging() {  
    ...
    // PT - 1024 Page Table dans Page Directory
    for(int i=0; i<1024; i++){
        PageTable new_page_table = (PageTable)kmalloc_a(sizeof(PTE) * 1024);
        memset(new_page_table,0,sizeof(PTE)*1024);                              //Init toutes les lignes à 0
        pageDirectory[i].value = (uint32_t)new_page_table|PAGE_PRESENT|PAGE_RW;  //Ajoute la table de page au page directory
        index = (uint32_t)new_page_table + sizeof(PTE) * 1024;                  //Ajoute la taille de la page pour l'adresse de la suivante
    }
...
PageTable alloc_page_entry(uint32_t address, int is_writeable, int is_kernel){
    ...
    page_table = (address>>12) & 0x3FF; // on recupere l'adresse de la page de table dans le répertoire de page
        
    uint32_t phy_page = findfreePage();// recherche d'une page libre dans la memoire physique

    int idx_pagetab = address & 0x7FF; // calcul de la position de la page dans la table de page
